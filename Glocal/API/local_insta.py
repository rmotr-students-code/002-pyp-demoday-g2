...
def get_local_instagram(st_num,st_name,st_type,city,state,miles):
    ...
    api = InstagramAPI(client_id=Insta_Client_ID, client_secret=Insta_Client_Secret)
    # queries instagram images using Instagram API with geographic parameters latitude and longitude as floats, and radius as a string
    local_media = api.media_search(count=20, lat=latitude, lng=longitude, distance=str((miles))+"km")
    # appends list of image links to 'photos' list. The image links are to 'standard resolution' versions of images, not thumbnails.
    photos = []
